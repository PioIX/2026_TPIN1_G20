...
class UserInterface {
    ...
    changeScreen() {
        ...
        if (notepad.style.display !== "none") {
            ...
            // Antes esto llamaba siempre a this.clearAllNotes() / this.clearSelect(),
            // pero #allNotes y #selectNote no existen en este HTML y tiraban error,
            // cortando la ejecución (por eso logout() quedaba a mitad de camino).
            if (document.getElementById("allNotes")) this.clearAllNotes();
            if (document.getElementById("selectNote")) this.clearSelect();
        }
        else {
    ...
    /*preguntar porque se sobreescriben las notas*/
    ...
    allNotesContainer.insertAdjacentHTML('beforeend', `
        <div id="note${id}" class="card shadow-sm m-3">
            <div class="card-header">
                <h5 class="card-title" id="noteTitle${id}">${title}</h5>
                <h6 class="card-subtitle mb-0 text-muted">ID: ${id}</h6>
